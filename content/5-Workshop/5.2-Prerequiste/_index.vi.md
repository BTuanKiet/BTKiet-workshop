...
Trước khi chạy `sam build`, cần sao chép hai tệp trọng số mô hình đã huấn luyện từ thư mục `ai-service/` vào `cloud-backend/functions/ai-inference/`:
...
Các tệp này được AI Inference Lambda sử dụng để chẩn đoán bệnh cây trồng thông qua mô hình ensemble ResNet50 và LeNet được huấn luyện trên bộ dữ liệu PlantVillage (38 lớp). Nếu thiếu các tệp này, `sam build` sẽ thất bại ở bước xây dựng Docker image.
